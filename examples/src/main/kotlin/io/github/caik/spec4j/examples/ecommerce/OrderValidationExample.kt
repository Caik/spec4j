...
data class OrderContext(
    ...
    val customerRiskScore: Double  // 0.0 = safe, 1.0 = high risk
)
...
object OrderSpecs {
    ...
    val hasItems = Specification.of<OrderContext, OrderFailureReason>(
        "HasItems",
        { it.items.isNotEmpty() },
        OrderFailureReason.EMPTY_CART
    )

    val withinMaxItems = Specification.of<OrderContext, OrderFailureReason>(
        "WithinMaxItems",
    ...
    val hasValidAddress = Specification.of<OrderContext, OrderFailureReason>(
        "HasValidAddress",
    ...
    val isBlockedCountry = Specification.of<OrderContext, OrderFailureReason>(
        "IsBlockedCountry",
        { it.shippingAddress?.country in listOf("XX", "YY", "ZZ") },
        OrderFailureReason.BLOCKED_COUNTRY  // This reason is used when NOT is applied
    )

    val hasValidPayment = Specification.of<OrderContext, OrderFailureReason>(
        "HasValidPayment",
        { it.paymentMethod?.expired == false },
        OrderFailureReason.PAYMENT_METHOD_EXPIRED
    )

    val hasSufficientFunds = Specification.of<OrderContext, OrderFailureReason>(
        "HasSufficientFunds",
        { (it.paymentMethod?.availableBalance ?: 0.0) >= 10.0 },
        OrderFailureReason.INSUFFICIENT_FUNDS
    )

    val isLowRisk = Specification.of<OrderContext, OrderFailureReason>(
        "IsLowRisk",
...
fun main() {
    ...
    val orderValidationPolicy = Policy.create<OrderContext, OrderFailureReason>()
    ...
    val outOfStockOrder = OrderContext(
        items = listOf(
            OrderItem("SKU-001", 2, false),  // out of stock
            OrderItem("SKU-002", 1, true),
            OrderItem("SKU-003", 3, false)   // out of stock
        ),
        shippingAddress = ShippingAddress("US", true),
        paymentMethod = PaymentMethod("CREDIT", false, 500.0),
    ...
    val blockedCountry = validOrder.copy(
        shippingAddress = ShippingAddress("XX", true)  // XX is blocked
    )
    ...
    val multipleIssues = OrderContext(
...
private fun runTest(name: String, context: OrderContext, policy: Policy<OrderContext, OrderFailureReason>) {
